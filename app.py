...
def my_store():
    ...
    if request.method == 'POST':
        ...
        gurupay_key = request.form.get("gurupay_key", "").strip()
        ...
        if store:
            db.stores.update_one({"user_id": user_id}, {"": data})
        else:
...
def store_index(store_id):
    store = db.stores.find_one({"store_id": store_id, "is_active": True})
    if not store: return "Store not found or inactive", 404
    
    raw_products = list(db.products.find({}).sort("order", 1))
    store_products = list(db.store_products.find({"store_id": store_id}))
    sp_dict = {p["pid"]: p["reseller_price"] for p in store_products}
    
    # Group products like in api.html
    grouped = {}
    for p in raw_products:
        p["display_price"] = sp_dict.get(p.get("id")) or p.get("price")
        name = p.get("name", "Unknown")
        cat = p.get("category", "Uncategorized")
        key = f"{cat}___{name}"
        if key not in grouped: grouped[key] = {"name": name, "category": cat, "plans": []}
        grouped[key]["plans"].append(p)
        
    return render_template("storefront/index.html", store=store, grouped_products=grouped)

@app.route('/store/<store_id>/buy/<pid>', methods=['GET'])
def store_buy(store_id, pid):
    store = db.stores.find_one({"store_id": store_id, "is_active": True})
    ...
    # Calculate price
    sp = db.store_products.find_one({"store_id": store_id, "pid": pid})
    reseller_price = sp["reseller_price"] if sp else product.get("price", 0)
    original_price = product.get("price", 0)
    
    # Check if reseller has sufficient balance in Main Server!
    reseller = db.users.find_one({"user_id": store["user_id"]})
    if not reseller or reseller.get("balance", 0) < original_price:
        return "Store owner has insufficient funds to process this order. Please contact support.", 400
        
    # Generate GuruPay Order on Reseller's Gateway
    import uuid
    order_id = "RES_" + str(uuid.uuid4().hex)[:10].upper()
    
    if not store.get("gurupay_key"):
        return "Store owner has not configured payment gateway.", 400
        
    payload = {
        "key": store.get("gurupay_key"),
        "client_txn_id": order_id,
        "amount": f"{reseller_price:.2f}",
        "p_info": f"{store.get('store_name')} - {product.get('name')} {product.get('plan_name')}",
        "customer_name": "Store Customer",
        "customer_email": "customer@karan.com",
        "customer_mobile": "9999999999",
        "redirect_url": request.url_root.rstrip('/') + f"/store/{store_id}/verify/{order_id}?pid={pid}"
    }
    
    try:
        r = requests.post(KARANPAY_CREATE_URL, data=payload, timeout=10)
        data = r.json()
        if data.get("status"):
            payment_url = data["data"]["payment_url"]
            return redirect(payment_url)
        else:
            return "Payment gateway error: " + data.get("msg", "Unknown"), 400
    except Exception as e:
        return "Payment gateway connection error: " + str(e), 400

@app.route('/store/<store_id>/verify/<order_id>')
def store_verify(store_id, order_id):
    ...
    # Check GuruPay Status
    try:
        today_date = datetime.now().strftime("%d-%m-%Y")
        r = requests.post(KARANPAY_STATUS_URL, data={"key": store.get("gurupay_key"), "client_txn_id": order_id, "txn_date": today_date}, timeout=10)
        data = r.json()
        if data.get("status") and data["data"]["status"] == "success":
            # Payment Successful!
            utr = data["data"].get("upi_txn_id", "N/A")
            
            # Now, deduct balance from Reseller and generate key
            original_price = product.get("price", 0)
            reseller = db.users.find_one({"user_id": store["user_id"]})
            
            if reseller.get("balance", 0) < original_price:
                return "Payment Received by Store Owner, but owner has insufficient balance to generate key! UTR: " + utr, 400
                
            # Deduct balance
            db.users.update_one({"user_id": reseller["user_id"]}, {"": {"balance": -original_price}})
            
            # Fetch Key from external API (adminpanels.shop)
            try:
                import tls_client
                ...
                req_data = {
                    ...
                    "action": "buy",
                    "product_id": pid,
                    "duration": product.get("plan_name", "")
                }
                res = session.post(API_ENDPOINT, data=req_data, timeout_seconds=20)
                api_resp = res.json()
                if api_resp.get("status") == "success":
                    key_data = api_resp.get("key", "N/A")
                    
                    # Log order for reseller
                    db.store_orders.insert_one({
                        "store_id": store_id,
                        "order_id": order_id,
                        "utr": utr,
                        "pid": pid,
                        "key_delivered": key_data,
                        "date": datetime.now()
                    })
                    
                    return render_template("storefront/success.html", store=store, key=key_data, product=product)
                else:
                    return f"Key generation failed (API Error: {api_resp.get('message')}). Store owner's balance was deducted.", 500
            except Exception as e:
                return f"Key generation failed (Connection Error). Store owner's balance was deducted.", 500
        else:
            return "Payment Pending or Failed. Please check again later."
    except Exception as e:
        return "Verification Error: " + str(e), 500
